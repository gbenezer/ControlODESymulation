...
### Initial Conditions and Custom Fields: Always Store
...
3. **Is it an initial condition or custom field?**
   - Used in logic or custom methods? → **Yes** (`self.C_A0`, `self.reactor_volume`)
   - Temporary calculation? → **No** (keep as local variable)

### Summary Table

| Item | Store as Attribute? | Example |
|------|---------------------|---------|
| State symbols (`C_A`, `T`) | ❌ No | Keep local, use `self.state_vars` |
| Control symbols (`Q`) | ❌ No | Keep local, use `self.control_vars` |
| Parameter symbols (`T_amb`) | ❌ No (usually) | Keep local, use as dict keys |
| Parameter values (for equilibria) | ✅ Yes | `self.T_amb_val = T_amb_val` |
| Parameter values (not used later) | ❌ No | Just put in `self.parameters` |
| Initial conditions (`C_A0`, `T0`) | ✅ Yes | `self.C_A0 = C_A0` |
| Custom application data | ✅ Yes | `self.reactor_volume = 10.0` |
| Temporary calculations | ❌ No | `r1 = k1 * C_A * exp(...)` |
...
Both are valid modeling choices. Use controlled form when you plan to apply inputs; use autonomous form for analyzing free dynamics or when control is not part of the problem.
...
A multiplicative noise process (state-dependent diffusion):

```python
class GeometricBrownianMotion(ContinuousStochasticSystem):
    def define_system(self, sigma=0.2):
        # Asset price
        ...
        # ZERO DRIFT (pure diffusion version)
        self._f_sym = sp.zeros(1, 1)
        
        # STATE-DEPENDENT DIFFUSION: dS = σ·S·dW
        self.diffusion_expr = sp.Matrix([[sigma_sym * S]])
        
        self.sde_type = "ito"
```

Note: Classic geometric Brownian motion includes drift ($dS = \mu S\,dt + \sigma S\,dW$), but this shows the pure diffusion case for illustration.
...
2. **`self.control_vars`**: List of control variable symbols (empty list `[]` for autonomous systems—see the detailed Autonomous Systems callout below)
   ```python
...
4. **`self._f_sym`**: Symbolic expression for system dynamics (drift for SDEs). For pure diffusion systems with zero drift, see the detailed Pure Diffusion Systems callout above.
   ```python
   self._f_sym = sp.Matrix([dC_A_dt, dC_B_dt, dT_dt])
...
**What NOT to store:**
```python
# ❌ Don't store state/control symbols as attributes
...
def define_system(self, param1=1.0, param2=2.0):
    ...
    # DON'T store symbols - they're already in self.state_vars, self.parameters
...
- **Parameter access**: Store parameter values as attributes (`self.param_val`) for use in `setup_equilibria()`, don't try to look up via `self.parameters[sp.symbols("param")]` (creates new symbol, causes KeyError)

## Next Steps

**Tutorial 2: Simulation and Visualization** (Coming next)
...
**Tutorial 3: Linearization and Stability**
- Computing linearizations around equilibria
