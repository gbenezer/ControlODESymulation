...
fig = stochastic_system.plotter.plot_trajectory(
    ...
    x_batch,
    state_names=['θ (rad)', 'ω (rad/s)'],
    title=f'Stochastic Pendulum Monte Carlo (n={n_trials}, σ={stochastic_system.parameters[sp.Symbol("sigma")]})',
    show_std=True
)
fig.show()
```
...
controllers = {
    'P (K=1)': lambda x, t: -1.0 * x,
    'P (K=5)': lambda x, t: -5.0 * x,
    'No Control': None
}
...
for sigma in noise_levels:
    ...
    results_noise[f'σ={sigma}'] = {'t': result['t'], 'x': result['x']}
...
for i in range(n_trajectories):
    ...
    stochastic_trajectories.append(result['x'])
