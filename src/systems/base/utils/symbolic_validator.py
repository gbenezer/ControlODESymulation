...
class SymbolicValidator:
    ...
    def _validate_types(self):
        ...
        if hasattr(system, "_f_sym") and system._f_sym is not None:
            if not isinstance(system._f_sym, sp.Matrix):
                self._errors.append(f"_f_sym must be sp.Matrix, got {type(system._f_sym).__name__}")
        ...
        if hasattr(system, "_h_sym") and system._h_sym is not None:
            if not isinstance(system._h_sym, sp.Matrix):
                self._errors.append(f"_h_sym must be sp.Matrix, got {type(system._h_sym).__name__}")
        ...
        if hasattr(system, "order"):
            if not isinstance(system.order, int):
                self._errors.append(f"order must be int, got {type(system.order).__name__}")
    ...
    def _validate_symbols(self):
        ...
        if not (
            hasattr(system, "state_vars")
            and hasattr(system, "control_vars")
            and hasattr(system, "_f_sym")
            and hasattr(system, "parameters")
        ):
            return
        ...
        if not (
            system.state_vars
            and system.control_vars is not None
        ...
        if system.parameters and not all(
            isinstance(k, sp.Symbol) for k in system.parameters.keys()
        ):
            return
    ...
    def _validate_naming_conventions(self):
        ...
        if hasattr(system, "order") and system.order == 2:
            if hasattr(system, "state_vars") and len(system.state_vars) >= 2:
                ...
                has_dot_pattern = any(
                    "dot" in name.lower() or "'" in name for name in state_names[nq:]
                )
    ...
    def _check_usage_patterns(self):
        ...
        if (
            hasattr(system, "_f_sym")
            and system._f_sym is not None
            and hasattr(system, "state_vars")
            and system.state_vars
        ):
