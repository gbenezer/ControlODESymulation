...
def _benchmark_backend(backend: str, device: str, operation: str, size: int) -> float:
    ...
    if backend == 'numpy':
        ...
        arr = jax.device_put(
            jax.random.normal(jax.random.PRNGKey(0), (size, size)),
            jax_device
        )
...
if __name__ == "__main__":
    ...
    for backend in ['numpy', 'pytorch', 'jax']:
        for device in ['cpu', 'gpu']:
            ...
            print(f"{status} {backend:10s} on {device:5s}: {'Working' if result else 'Not available'}")
    ...
    try:
        ...
        # Sort by speed
        sorted_times = sorted(
            [(k, v) for k, v in times.items() if isinstance(v, float)],
            key=lambda x: x[1]
        )
