...
@pytest.fixture
def quadrotor_lidar_dt(quadrotor_ct):
    """Create a quadrotor system with LIDAR observation using builtin class"""
    return GenericDiscreteTimeSystem(
        quadrotor_ct,
        dt=0.001,
        integration_method=IntegrationMethod.RK4,
        position_integration=IntegrationMethod.MidPoint,
    )

@pytest.fixture
def vdp_oscillator_dt(vdp_ct):
    """Create a VdP Oscillator system with builtin class"""
    return GenericDiscreteTimeSystem(vdp_ct, dt=0.01, integration_method=IntegrationMethod.MidPoint)
...
def pendulum_dt_rk4(pendulum_ct):
    ...
    return GenericDiscreteTimeSystem(pendulum_ct, dt=0.01, integration_method=IntegrationMethod.RK4)
...
class TestSimulation:
    ...
    def test_simulate_with_nn_controller(self, pendulum_dt):
        ...
        controller_nn = torch.nn.Sequential(
            torch.nn.Linear(2, 10), torch.nn.Tanh(), torch.nn.Linear(10, 1)
        )
    ...
    def test_simulate_return_final_only(self, pendulum_dt):
        ...
        x_final = pendulum_dt.simulate(x0, controller=None, horizon=10, return_all=False)
    ...
    def test_simulate_with_observer(self, partial_obs_pendulum_ct):
        ...
        traj = system.simulate(x0, controller=controller, observer=observer, horizon=20)
...
class TestVisualization:
    ...
    def test_plot_trajectory_with_controls(self, pendulum_dt):
        ...
        fig = pendulum_dt.plot_trajectory(traj, control_sequence=controls, show=False)
    ...
    def test_plot_trajectory_batched(self, pendulum_dt):
        ...
        fig = pendulum_dt.plot_trajectory(
            traj, trajectory_names=["Traj 1", "Traj 2", "Traj 3"], show=False
        )
    ...
    def test_plot_trajectory_custom_names(self, pendulum_dt):
        ...
        fig = pendulum_dt.plot_trajectory(
            traj, state_names=["θ (rad)", "θ̇ (rad/s)"], title="Pendulum Motion", show=False
        )
    ...
    def test_plot_trajectory_save_html(self, pendulum_dt, tmp_path):
        ...
        fig = pendulum_dt.plot_trajectory(traj, save_html=str(filepath), show=False)
    ...
    def test_plot_phase_portrait_2d(self, pendulum_dt):
        ...
        fig = pendulum_dt.plot_phase_portrait_2d(
            traj, state_indices=(0, 1), state_names=("θ", "θ̇"), show=False
        )
    ...
    def test_plot_phase_portrait_2d_batched(self, pendulum_dt):
        ...
        fig = pendulum_dt.plot_phase_portrait_2d(
            traj, trajectory_names=["IC1", "IC2", "IC3"], show=False
        )
