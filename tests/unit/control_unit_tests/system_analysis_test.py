...
class AnalysisTestCase(unittest.TestCase):
    ...
    def assert_observability_info_valid(self, info: ObservabilityInfo):
        ...
        required_keys = ["observability_matrix", "rank", "is_observable", "unobservable_modes"]
        for key in required_keys:
            self.assertIn(key, info, f"Missing key: {key}")
...
class TestStabilityMethod(AnalysisTestCase):
    ...
    def test_stability_custom_tolerance(self):
        ...
        info = analyzer.stability(self.A_marginal, system_type="continuous", tolerance=1e-6)
...
class TestControllabilityMethod(AnalysisTestCase):
    ...
    def test_controllability_custom_tolerance(self):
        ...
        info = analyzer.controllability(self.A_ctrl, self.B_ctrl, tolerance=1e-6)
...
class TestObservabilityMethod(AnalysisTestCase):
    ...
    def test_observability_custom_tolerance(self):
        ...
        info = analyzer.observability(self.A_obs, self.C_obs, tolerance=1e-6)
...
class TestAnalyzeLinearization(AnalysisTestCase):
    ...
    def test_analyze_linearization_default_continuous(self):
        ...
        # Should work without explicit system_type
        result = analyzer.analyze_linearization(self.A_stable, self.B_stable, self.C_stable)
...
class TestBackendConsistency(AnalysisTestCase):
    ...
    def test_backend_not_mutated_by_methods(self):
        ...
        analyzer.analyze_linearization(self.A_stable, self.B_stable, self.C_stable)
...
class TestDelegation(AnalysisTestCase):
    ...
    def test_stability_delegates_correctly(self):
        ...
        with patch("src.control.classical_control_functions.analyze_stability") as mock_func:
            ...
            analyzer.stability(self.A_stable, system_type="continuous", tolerance=1e-8)

            # Verify correct delegation
            mock_func.assert_called_once_with(self.A_stable, "continuous", 1e-8)
    ...
    def test_controllability_delegates_correctly(self):
        ...
        with patch("src.control.classical_control_functions.analyze_controllability") as mock_func:
            ...
            analyzer.controllability(self.A_stable, self.B_stable, tolerance=1e-9)

            mock_func.assert_called_once_with(self.A_stable, self.B_stable, 1e-9)
    ...
    def test_observability_delegates_correctly(self):
        ...
        with patch("src.control.classical_control_functions.analyze_observability") as mock_func:
            ...
            analyzer.observability(self.A_stable, self.C_stable, tolerance=1e-7)

            mock_func.assert_called_once_with(self.A_stable, self.C_stable, 1e-7)
    ...
    def test_analyze_linearization_calls_all_methods(self):
        ...
        with (
            patch.object(analyzer, "stability") as mock_stab,
...
class TestIntegration(AnalysisTestCase):
    ...
    def test_individual_analyses_match_comprehensive(self):
        ...
        # Should match
        assert_allclose(stab["eigenvalues"], result["stability"]["eigenvalues"])
        self.assertEqual(ctrl["rank"], result["controllability"]["rank"])
        self.assertEqual(obs["rank"], result["observability"]["rank"])
...
class TestErrorPropagation(AnalysisTestCase):
    ...
    def test_analyze_linearization_propagates_errors(self):
        ...
        with self.assertRaises(ValueError):
            analyzer.analyze_linearization(A_wrong, self.B_stable, self.C_stable)
...
class TestInterface(AnalysisTestCase):
    ...
    def test_all_methods_exist(self):
        ...
        for method in required_methods:
            self.assertTrue(hasattr(analyzer, method), f"Missing method: {method}")
            self.assertTrue(callable(getattr(analyzer, method)), f"{method} is not callable")
