...
class TestCVXPYIntegration:
    ...
    def test_semidefinite_program(self):
        ...
        constraints = [X >> 0, X[0, 0] == 1]  # Positive semidefinite
...
class TestCasADiIntegration:
    ...
    def test_nonlinear_program_ipopt(self):
        ...
        # Solver options
        opts = {"ipopt.print_level": 0, "print_time": 0, "ipopt.sb": "yes"}  # Suppress banner

        # Create solver
        solver = ca.nlpsol("solver", "ipopt", nlp, opts)

        # Solve
        sol = solver(x0=[0.5, 0.5], lbg=-ca.inf, ubg=1.0)  # x^2 + y^2 <= 1
    ...
    def test_trajectory_optimization_pendulum(self):
        ...
        # Create NLP
        nlp = {"x": ca.vertcat(theta, omega, u), "f": cost, "g": ca.vertcat(*g_dyn)}
...
class TestCrossLibraryComparison:
    ...
    def test_quadratic_program_scipy_vs_cvxpy(self):
        ...
        constraints_cvx = [x >= 0, cp.sum(x) == 1]
