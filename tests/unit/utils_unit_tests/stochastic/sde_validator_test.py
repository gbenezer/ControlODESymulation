...
class TestDimensionValidation:
    ...
    def test_drift_dimension_mismatch(self, dimension_mismatch_drift):
        ...
        assert any(
            "drift" in err.lower() and ("dimension" in err.lower() or "rows" in err.lower())
    ...
    def test_diffusion_dimension_mismatch(self, dimension_mismatch_diffusion):
        ...
        assert not result.is_valid
        assert any(
            "diffusion" in err.lower() and "dimension" in err.lower() for err in result.errors
        )
...
class TestSymbolValidation:
    ...
    def test_undefined_symbol_in_drift(self, undefined_symbol_in_drift):
        ...
        assert any(
            ("undefined" in err.lower() or "unknown" in err.lower()) and "drift" in err.lower()
            for err in result.errors
        )
    ...
    def test_undefined_symbol_in_diffusion(self, undefined_symbol_in_diffusion):
        ...
        result = validator.validate()
        assert not result.is_valid
        assert any("undefined" in err.lower() or "unknown" in err.lower() for err in result.errors)
    ...
    def test_zero_diffusion_warning(self, zero_diffusion_system):
        ...
        if not result.is_valid:
            ...
            has_zero_warning = any(
                "zero" in warn.lower() and "diffusion" in warn.lower() for warn in result.warnings
            )
...
class TestParameterValidation:
    ...
    def test_unused_parameter_warning(self):
        ...
        validator = SDEValidator(drift, diffusion, [x], [u], parameters={alpha: 1.0, beta: 2.0})
    ...
    def test_nan_parameter_error(self):
        ...
        validator = SDEValidator(drift, diffusion, [x], [u], parameters={alpha: np.nan})

        result = validator.validate()
        assert not result.is_valid
        assert any("non-finite" in err.lower() or "nan" in err.lower() for err in result.errors)
    ...
    def test_inf_parameter_error(self):
        ...
        validator = SDEValidator(drift, diffusion, [x], [u], parameters={alpha: np.inf})

        result = validator.validate()
        assert not result.is_valid
        assert any("non-finite" in err.lower() or "inf" in err.lower() for err in result.errors)
...
class TestEdgeCases:
    ...
    def test_large_system(self):
        ...
        drift = sp.Matrix([-x + u if i == 0 else -x for i, x in enumerate(state_vars)])
        #                ^                                 ^
        #                Single brackets - creates (10, 1) column vector

        # Diagonal diffusion
        diffusion = sp.Matrix([[0.1 if i == j else 0.0 for j in range(n)] for i in range(n)])
    ...
    def test_complex_symbolic_expressions(self):
        ...
        drift = sp.Matrix([[x2 - sp.sin(x1)], [-x1 + u * sp.cos(x2)]])
