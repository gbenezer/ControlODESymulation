...
class TestDiscretizedSystemInit:
    ...
    def test_integrator_kwargs_stored(self):
        ...
        continuous = MockContinuousSystem()
        discrete = DiscretizedSystem(continuous, dt=0.01, method="RK45", rtol=1e-9, atol=1e-11)
...
class TestHelperFunctions:
    ...
    def test_recommend_dt(self):
        ...
        rec = recommend_dt(
            continuous,
            x0,
            target_error=1e-4,  # Relaxed tolerance
            method="rk4",
            dt_range=(0.005, 0.05),
            n_test=5,  # Narrower range
        )
    ...
    def test_compute_discretization_quality(self):
        ...
        quality = compute_discretization_quality(discrete, np.array([1.0, 0.0]), None, n_steps=50)
...
class TestInterpolation:
    ...
    def test_cubic_interpolation_with_sufficient_points(self):
        ...
        discrete = DiscretizedSystem(
            ...
            interpolation_kind="cubic",
            rtol=1e-6,
            atol=1e-9,  # Tighter tolerances -> more adaptive points
        )
...
class TestStochasticSimulation:
    ...
    def test_simulate_stochastic_returns_correct_structure(self):
        ...
        discrete = DiscretizedSystem(stochastic, dt=0.01, sde_method="euler_maruyama")
