...
def batched_trajectory_2d():
    ...
    t = np.linspace(0, 2 * np.pi, 100)
    x_batch = np.stack(
        [np.column_stack([(1 + 0.2 * i) * np.cos(t), (1 + 0.2 * i) * np.sin(t)]) for i in range(3)],
    )  # (3, 100, 2)
    return x_batch
...
class TestPlot2D:
    ...
    def test_custom_state_names(self, plotter, circular_trajectory_2d):
        ...
        fig = plotter.plot_2d(circular_trajectory_2d, state_names=("Position", "Velocity"))
    ...
    def test_no_direction_arrows(self, plotter, circular_trajectory_2d):
        ...
        fig = plotter.plot_2d(circular_trajectory_2d, show_direction=False)

        assert isinstance(fig, go.Figure)

    def test_no_start_end_markers(self, plotter, circular_trajectory_2d):
        """Test without start/end markers."""
        fig = plotter.plot_2d(circular_trajectory_2d, show_start_end=False)

        assert isinstance(fig, go.Figure)

    def test_with_vector_field(self, plotter, circular_trajectory_2d, simple_vector_field):
        """Test with vector field overlay."""
        fig = plotter.plot_2d(circular_trajectory_2d, vector_field=simple_vector_field)

        assert isinstance(fig, go.Figure)

    def test_with_equilibria(self, plotter, circular_trajectory_2d):
        """Test with equilibrium points."""
        equilibria = [np.array([0.0, 0.0]), np.array([1.0, 0.0])]

        fig = plotter.plot_2d(circular_trajectory_2d, equilibria=equilibria)
    ...
    def test_custom_title(self, plotter, circular_trajectory_2d):
        ...
        fig = plotter.plot_2d(circular_trajectory_2d, title=custom_title)

        assert fig.layout.title.text == custom_title

    def test_color_scheme(self, plotter, batched_trajectory_2d):
        """Test with different color schemes."""
        fig = plotter.plot_2d(batched_trajectory_2d, color_scheme="colorblind_safe")

        assert isinstance(fig, go.Figure)

    def test_theme_application(self, plotter, circular_trajectory_2d):
        """Test theme application."""
        fig = plotter.plot_2d(circular_trajectory_2d, theme="publication")
...
class TestPlot3D:
    ...
    def test_custom_state_names(self, plotter, lorenz_trajectory_3d):
        """Test with custom state names."""
        fig = plotter.plot_3d(lorenz_trajectory_3d, state_names=("x", "y", "z"))
    ...
    def test_batched_3d_portrait(self, plotter):
        ...
        x_batch = np.stack(
            [np.column_stack([np.sin(t + phi), np.cos(t + phi), t * 0.1]) for phi in [0, 0.5, 1.0]],
        )  # (3, 200, 3)
    ...
    def test_no_direction_markers(self, plotter, lorenz_trajectory_3d):
        ...
        fig = plotter.plot_3d(lorenz_trajectory_3d, show_direction=False)

        assert isinstance(fig, go.Figure)

    def test_no_start_end_markers(self, plotter, lorenz_trajectory_3d):
        """Test without start/end markers."""
        fig = plotter.plot_3d(lorenz_trajectory_3d, show_start_end=False)
    ...
    def test_custom_title(self, plotter, lorenz_trajectory_3d):
        ...
        fig = plotter.plot_3d(lorenz_trajectory_3d, title=custom_title)
    ...
    def test_color_scheme(self, plotter):
        ...
        fig = plotter.plot_3d(x_batch, color_scheme="tableau")

        assert isinstance(fig, go.Figure)

    def test_theme_application(self, plotter, lorenz_trajectory_3d):
        """Test theme application."""
        fig = plotter.plot_3d(lorenz_trajectory_3d, theme="dark")
...
class TestPlotLimitCycle:
    ...
    def test_custom_state_names(self, plotter, circular_trajectory_2d):
        """Test with custom state names."""
        fig = plotter.plot_limit_cycle(circular_trajectory_2d, state_names=("x", "y"))

        assert isinstance(fig, go.Figure)

    def test_with_period_estimate(self, plotter, circular_trajectory_2d):
        """Test with period estimate."""
        fig = plotter.plot_limit_cycle(circular_trajectory_2d, period_estimate=50)
    ...
    def test_custom_title(self, plotter, circular_trajectory_2d):
        ...
        fig = plotter.plot_limit_cycle(circular_trajectory_2d, title=custom_title)

        assert fig.layout.title.text == custom_title

    def test_theme_application(self, plotter, circular_trajectory_2d):
        """Test theme application."""
        fig = plotter.plot_limit_cycle(circular_trajectory_2d, theme="publication")

        assert fig.layout.font.size == 14

    def test_color_scheme(self, plotter, circular_trajectory_2d):
        """Test with different color scheme."""
        fig = plotter.plot_limit_cycle(circular_trajectory_2d, color_scheme="colorblind_safe")
...
class TestVectorField:
    ...
    def test_vector_field_pendulum(self, plotter, spiral_trajectory_2d, pendulum_vector_field):
        ...
        fig = plotter.plot_2d(spiral_trajectory_2d, vector_field=pendulum_vector_field)
...
class TestEquilibria:
    ...
    def test_empty_equilibria_list(self, plotter, circular_trajectory_2d):
        ...
        fig = plotter.plot_2d(circular_trajectory_2d, equilibria=[])
